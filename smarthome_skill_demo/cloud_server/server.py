...
class Resources(db.Model):
    ...
    device_names = db.Column(db.String(100))
...
def api_valid():
    if request.method == 'GET':
        if 'valid' in request.args:
            token = load_token(request.args.get('access_token'))
            if token:
                response = {
                    'valid': True
                }
...
def api_user_dev():
    if request.method == 'GET':
        ...
        if token:
            ...
            if res_user:
                device_names         = res_user.device_names
                device_serial_number = res_user.device_serial_number
                response = {
